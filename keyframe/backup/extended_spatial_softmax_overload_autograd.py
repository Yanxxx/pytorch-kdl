...
class ExtendedSpatialSoftmax(nn.Module):
    ...
    def __init__(self, height, width, channel, image_height, image_width, 
        ...
        self.camera_intrinsic = torch.Tensor(camera_intrinsic).to('cuda:0')
...
#        print(output.shape)
        ...
        output = self.transform.apply(output, depth.shape[0], 
                                      self.channel,
                                      self.rotation,
                                      self.translation,
                                      self.br, self.bt)
...
#        print(Z.shape, xy.shape, xy[:, 0].shape)
        ...
        imsize = torch.Tensor([image_height, image_width]).to('cuda:0')
#        print(X.shape, Y.shape, Z.shape)
        feature_keypoints = torch.cat([X[None, :], Y[None, :], Z[None, :]], 0)
#        print(feature_keypoints.shape)
        ctx.save_for_backward(feature_keypoints, xy, camera_intrinsic, imsize)
        return feature_keypoints

    @staticmethod
    def backward(ctx, grad_output):
        output, xy, camera_intrinsic, imsize = ctx.saved_tensors
        image_height, image_width = imsize
        x = camera_intrinsic[2]
        y = camera_intrinsic[5]
        f_x = camera_intrinsic[0]
        f_y = camera_intrinsic[4]
        delta_xyz = grad_output.clone()
        target = output + delta_xyz
        x = target[0, :]
        y = target[1, :]
        z = target[2, :]
...
#        print('Transformation network input: ', data.shape)
        reprojected_pt = torch.matmul(rotation, data) + translation#[:,None]  
        ...
        ctx.save_for_backward(ri, ti)
        return reprojected_pt
    
    @staticmethod
    def backward(ctx, data):
        size = list(data.shape)
        br, bt = ctx.saved_tensors
        data = torch.transpose(data.reshape(size[0], size[1] // 3, 3), 2, 1)        
        split_data = torch.split(data, 1, dim = 0)
        data = torch.squeeze(torch.cat(split_data, 2))
        reprojected_pt = torch.matmul(br, data) + bt#[:,None]
        
        return reprojected_pt, None, None, None, None, None, None
